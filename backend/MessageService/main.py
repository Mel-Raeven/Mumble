...
def post():
    ...
    ___sql_statement = 'INSERT INTO Message (Content) VALUES (%s);'
    ___variables = (
...
def get():
    ...
    ___sql_statement = 'SELECT MessageID from MessageUser where UserID=%s AND User2ID=%s;'
    ___variables = (
        userID,
        friendID,
    )
    ...
    for id in tuple(list):
        ___sql_statement = 'SELECT content from Message where id=%s;'
        ___variables=(
            id,
        )
        create_cursor.execute(___sql_statement, ___variables)
